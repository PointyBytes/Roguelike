...
class Sample(tcod.event.EventDispatch[None]):
    ...
    def ev_keydown(self, event: tcod.event.KeyDown) -> None:
        ...
        if event.sym == tcod.event.KeySym.DOWN:
            ...
            event.sym == tcod.event.KeySym.PRINTSCREEN
            or event.sym == tcod.event.KeySym.p
        ):
            print("screenshot")
...
class TrueColorSample(Sample):
    def __init__(self) -> None:
        ...
        self.slide_dir: NDArray[np.int16] = np.array(
    ...
    def on_draw(self) -> None:
        ...
        for x in range(sample_console.width):
            ...
            libtcodpy.console_set_char_background(
                sample_console, x, rect_y, col, self.bk_flag
            )
            libtcodpy.console_set_char_background(
                sample_console, x, rect_y + 1, col, self.bk_flag
            )
            libtcodpy.console_set_char_background(
                sample_console, x, rect_y + 2, col, self.bk_flag
            )
        angle = time.time() * 2.0
...
class FOVSample(Sample):
    def __init__(self) -> None:
        ...
        self.light_map_bg: NDArray[np.uint8] = np.full(
            SAMPLE_MAP.shape, LIGHT_GROUND, dtype="3B"
        )
        self.light_map_bg[SAMPLE_MAP[:] == "#"] = LIGHT_WALL
        # Dark background colors for the map.
        self.dark_map_bg: NDArray[np.uint8] = np.full(
            SAMPLE_MAP.shape, DARK_GROUND, dtype="3B"
        )
...
class PathfindingSample(Sample):
    ...
    def on_draw(self) -> None:
        ...
        for y in range(SAMPLE_SCREEN_HEIGHT):
            for x in range(SAMPLE_SCREEN_WIDTH):
                if SAMPLE_MAP[x, y] == "#":
                    libtcodpy.console_set_char_background(
                        sample_console, x, y, DARK_WALL, libtcodpy.BKGND_SET
                    )
                else:
                    libtcodpy.console_set_char_background(
                        sample_console, x, y, DARK_GROUND, libtcodpy.BKGND_SET
                    )
        # draw the path
        if self.using_astar:
            for i in range(libtcodpy.path_size(self.path)):
                ...
                libtcodpy.console_set_char_background(
                    sample_console, x, y, LIGHT_GROUND, libtcodpy.BKGND_SET
                )
        else:
            for y in range(SAMPLE_SCREEN_HEIGHT):
                for x in range(SAMPLE_SCREEN_WIDTH):
                    if SAMPLE_MAP[x, y] != "#":
                        libtcodpy.console_set_char_background(
                            ...
                            libtcodpy.color_lerp(  # type: ignore
                                ...
                                DARK_GROUND,
                                0.9
                                * libtcodpy.dijkstra_get_distance(self.dijkstra, x, y)
                                / self.dijkstra_dist,
                            ),
                            libtcodpy.BKGND_SET,
                        )
        ...
        if self.busy <= 0.0:
            ...
            if self.using_astar:
                if not libtcodpy.path_is_empty(self.path):
                    libtcodpy.console_put_char(
                        sample_console, self.px, self.py, " ", libtcodpy.BKGND_NONE
                    )
    ...
    def ev_keydown(self, event: tcod.event.KeyDown) -> None:
        if event.sym == tcod.event.KeySym.i and self.dy > 0:
            ...
            libtcodpy.console_put_char(
                sample_console, self.dx, self.dy, self.old_char, libtcodpy.BKGND_NONE
            )
            self.dy -= 1
            self.old_char = sample_console.ch[self.dx, self.dy]
            libtcodpy.console_put_char(
                sample_console, self.dx, self.dy, "+", libtcodpy.BKGND_NONE
            )
...
def traverse_node(bsp_map: NDArray[np.bool_], node: tcod.bsp.BSP) -> None:
    if not node.children:
        ...
        if node.horizontal:
            ...
            left.y + left.height - 1 < right.y or right.y + right.height - 1 < left.y
        ):  # horizontal corridor
            # no overlapping zone. we need a Z shaped corridor
...
class BSPSample(Sample):
    def __init__(self) -> None:
        ...
        self.bsp_map: NDArray[np.bool_] = np.zeros(
    ...
    def on_draw(self) -> None:
        ...
        for y in range(SAMPLE_SCREEN_HEIGHT):
            for x in range(SAMPLE_SCREEN_WIDTH):
                ...
                libtcodpy.console_set_char_background(
                    sample_console, x, y, color, libtcodpy.BKGND_SET
                )
    ...
    def on_draw(self) -> None:
        ...
        if int(time.time()) % 2:
            ...
            self.circle.blit_rect(
                sample_console, 0, 3, -1, -1, libtcodpy.BKGND_MULTIPLY
            )
            # the green channel
            sample_console.draw_rect(15, 3, 15, 15, 0, None, (0, 255, 0))
            self.circle.blit_rect(
                sample_console, 15, 3, -1, -1, libtcodpy.BKGND_MULTIPLY
            )
            # the blue channel
            sample_console.draw_rect(30, 3, 15, 15, 0, None, (0, 0, 255))
            self.circle.blit_rect(
                sample_console, 30, 3, -1, -1, libtcodpy.BKGND_MULTIPLY
            )
        else:
...
class FastRenderSample(Sample):
    ...
    def on_draw(self) -> None:
        ...
        self.lights = [
            light for light in self.lights if light.z - time_delta > 1.0 / RES_V
        ]
        ...
        for light in self.lights:  # render lights
            ...
            light_brightness = (
                LIGHT_BRIGHTNESS * light.strength * (1.0 - light.z / TEX_STRETCH)
            )
        ...
        if context.sdl_renderer:
            ...
            if isinstance(event, (tcod.event.MouseState, tcod.event.MouseMotion)):
                event.tile = tcod.event.Point(
                    event.position.x // tileset.tile_width,
                    event.position.y // tileset.tile_height,
                )
